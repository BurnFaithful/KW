...
x2_train, x2_test, y2_train, y2_test, y3_train, y3_test = train_test_split(
    x2, y2, y3, random_state=66, test_size=0.4, shuffle=False
)

x2_val, x2_test, y2_val, y2_test, y3_val, y3_test = train_test_split(
    x2_test, y2_test, y3_test, random_state=66, test_size=0.5, shuffle=False
)
...
model.fit([x1_train, x2_train], [y1_train, y2_train, y3_train], 
            epochs=120, 
            batch_size=1,
            validation_data=([x1_val, x2_val], [y1_val, y2_val, y3_val])) # 검증

# fit을 여러 번 해도 각각은 독립시행임.

#4. 평가 예측
loss = model.evaluate([x1_test, x2_test], [y1_test, y2_test, y3_test], batch_size=1) # mean squared error
# print("mse :", mse)
print("loss :", loss)
