# 1 input, 2 output ensemble model
...
x1_train, x1_test, y1_train, y1_test, y2_train, y2_test = train_test_split(
    x1, y1, y2, random_state=66, test_size=0.4, shuffle=False
)

x1_val, x1_test, y1_val, y1_test, y2_val, y2_test = train_test_split(
    x1_test, y1_test, y2_test, random_state=66, test_size=0.5, shuffle=False
)
...
model.fit(x1_train, [y1_train, y2_train], 
            epochs=120, 
            batch_size=1,
            validation_data=(x1_val, [y1_val, y2_val])) # 검증

# fit을 여러 번 해도 각각은 독립시행임.

#4. 평가 예측
loss = model.evaluate(x1_test, [y1_test, y2_test], batch_size=1) # mean squared error
# print("mse :", mse)
print("loss :", loss)
